...
export async function POST(request: NextRequest) {
  ...
  const { userId, workspaceId, userRequest, fileData, modelId = "gemini-3.5-pro" } = body as {
    userId: string;
    workspaceId: string;
  ...
  if (!finalModelId.startsWith("gemini-")) {
    return Response.json({ message: "This model is currently only available for App Generation, not the Improve tool." }, { status: 400 });
  }
